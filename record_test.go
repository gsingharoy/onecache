...
func TestRecordHasExpired(t *testing.T) {
	...
	t.Log("When the record has an expiredIn time -1")
	r3 := &record{expiresIn: -1, timestamp: time.Now().Add(time.Second * time.Duration(11000) * -1)}
	assert.Equal(t, false, r3.hasExpired())
}
